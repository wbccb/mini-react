...
初始化`valueCursor={current: null}`，每次调用`pushProvider()`更新目前Context最新的值到栈中，并且更新`context._currentValue`的值
...
本质就是`</Context.Provider>`，也可以说是执行`completeWork()`

![Image](https://github.com/user-attachments/assets/c04e7a82-193e-4412-98ee-46de1eab83b0)

如上图所示，我们在`completeWork()`触发对应的`popProvider()`->`pop()`
...
beginWork()=>`<Context.Provide test={333}>`形成新的类型=>触发对应的处理:
- 首次渲染/更新渲染时，将Context加入stack中
- 更新时，通知使用useContext()的fiber更新 => 比较Context内容，深度遍历fiber.children，找到fiber.dependency，检测是不是目前的Context，进行update标记（lanes标记）
  beginWork()=>触发FunctionComponent的renderWithHoos()=>触发useContext():
- 预构建：初始化fiber.dependency结构
- useContext()->readContext，注册当前Context到fiber.dependency中，将拿到的Context的值返回
  Commit阶段:
- 弹出stack中Context的值
  结束阶段:
- 重置所有状态